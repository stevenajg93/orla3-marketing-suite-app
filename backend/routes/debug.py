    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            try:
                ...
                if "step_2_database" not in results or not results["step_2_database"]["success"]:
                    results["step_2_database"] = {"success": False, "error": str(e)}
                    results["errors"].append(f"Step 2 failed: {str(e)}")
                elif "step_4_selected_folders_column" not in results:
...
async def debug_user_data(email: str):
    ...
    try:
        ...
        result["errors"].append(f"Database connection failed: {type(e).__name__}: {str(e)}")
